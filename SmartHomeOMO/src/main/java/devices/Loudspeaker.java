...
public class Loudspeaker extends Device {
    ...
    private final static int maxUsingDuration = 5;

    public Loudspeaker(String name, String brand, Room location, ConsumptionType consumptionType, double[] consumption) {
     ...
     * @return using state
     *
     * @see Loudspeaker#playingNotify(Person)
     */
    @Override
     ...
     */
    private void playingNotify(Person person) {
        newNotification(new Notification(NotificationType.LISTENING_MUSIC, getActualFloor(), getActualRoom(), person,this));
    }
     ...
     * @return using state
     */
    private Usable useLoudspeaker(Person person){
        if (actualUsingDuration < maxUsingDuration) {
            if (actualUsingDuration == 0)
                playingNotify(person);
            actualUsingDuration++;
            return this;
        } else {
